...
```
**推荐改法**：把 `ROLE_VARIANT` 显式声明 `Record<string, BadgeProps["variant"]>`，去掉 `as never` 用 `??` 兜底。
...
短 fragment `<>` 不接受 props（含 key）。`map` 在外面，React 必报「Each child in a list should have a unique key prop」。
**推荐改法**：用具名 fragment：`<React.Fragment key={perm.label}>…</React.Fragment>`。
...
```
sidebar `< md` 隐藏（topbar 后续加抽屉 trigger）；main padding 收紧。
...
nav 内容 ≈ 480px + 头 + padding + 底部卡 124px，合计 ≈ 676px。当 `h-screen < 700px`（笔记本横屏 16:10 = 800×500 也可能）时**底部套餐卡叠到「设置」导航项上**，且 aside 没 `overflow-y-auto`。

**推荐改法**：sidebar 改 flex 列、底部用 `mt-auto`、外层 `overflow-y-auto`：
```tsx
<aside className="sticky top-0 h-screen w-[248px] ... overflow-y-auto flex flex-col">
  <Link>...</Link>
  <nav className="mt-6 flex flex-col gap-1">...</nav>
  <div className="mt-auto rounded-lg ..."> {/* 删 absolute / bottom-4 */}
</aside>
```
